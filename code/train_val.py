...
'''
Commands for replication of the four models:

- baseline
python [dir_to_this_file] -init_tau 2.0 -use_max_pool -device cuda:0 -dataset_name DVS128Gesture -log_dir_prefix [dataset_directory] -T 20 -max_epoch 1256 -detach_reset -channels 128 -number_layer 5 -split_by number -normalization None -use_plif

- No regularization
python [dir_to_this_file] -init_tau 2.0 -tau_std_dev 0.1 -use_max_pool -device cuda:0 -dataset_name DVS128Gesture -log_dir_prefix [dataset_directory] -T 20 -max_epoch 1256 -detach_reset -channels 128 -number_layer 5 -split_by number -normalization None -use_plif -tau_per_neuron

- lambda=0.01
python [dir_to_this_file] -init_tau 2.0 -tau_std_dev 0.1 -use_max_pool -device cuda:0 -dataset_name DVS128Gesture -log_dir_prefix [dataset_directory] -T 20 -max_epoch 1256 -detach_reset -channels 128 -number_layer 5 -split_by number -normalization None -use_plif -tau_per_neuron -reg_lambda 0.01

- lambda=0.1
python [dir_to_this_file] -init_tau 2.0 -tau_std_dev 0.1 -use_max_pool -device cuda:0 -dataset_name DVS128Gesture -log_dir_prefix [dataset_directory] -T 20 -max_epoch 1256 -detach_reset -channels 128 -number_layer 5 -split_by number -normalization None -use_plif -tau_per_neuron -reg_lambda 0.1
'''
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tau_per_neuron', action='store_true', default=False) # Tau per each neuron (default False - only per each layer)
    parser.add_argument('-tau_std_dev', type=float, default=0) # Standard deviation from "init_tau" in case of log-normal initialization
    parser.add_argument('-reg_lambda', type=float, default=0) # Lambda regularization term
    ...
    dataset_dir = '/content/drive/MyDrive/Parametric-Leaky-Integrate-and-Fire-Spiking-Neuron/data/' + dataset_name # Dataset directory
    ...
    tau_per_neuron = args.tau_per_neuron
    tau_std_dev = args.tau_std_dev
    ...
    dir_name = f'val_{val_ratio}_{dataset_name}_init_tau_{init_tau}_tau_std_dev_{tau_std_dev}_use_plif_{use_plif}_use_max_pool_{use_max_pool}_T_{T}_c_{channels}_n_{number_layer}_split_by_{split_by}_normalization_{normalization}_detach_reset_{detach_reset}_tau_per_neuron_{tau_per_neuron}_lambda_reg_{reg_lambda}'
    ...
    if dataset_name == 'MNIST':
        ...
        parent_dir = os.path.join(current_dir, '..', 'spikingjelly')
    ...
    checkpoint_max_path = os.path.join(pt_dir, 'checkpoint_max.pt')
    ...
    if os.path.exists(checkpoint_path):
        ...
        if dataset_name == 'MNIST':
            net = models.MNISTNet(T=T, init_tau=init_tau, tau_std_dev=tau_std_dev, use_plif=use_plif, use_max_pool=use_max_pool,
            ...
            net = models.DVS128GestureNet(T=T, init_tau=init_tau, tau_std_dev=tau_std_dev, use_plif=use_plif, use_max_pool=use_max_pool,
                                          alpha_learnable=alpha_learnable, detach_reset=detach_reset, channels=channels,
                                          number_layer=number_layer).to(device)
    ...
    if checkpoint is not None:
        ...
        log_data_list = checkpoint['log_data_list']
        del checkpoint
    else:
        log_data_list = []
    ...
    for net.epoch in range(net.epoch, max_epoch):
        ...
        for img, label in train_data_loader:
            ...
            # Regularization term for individual PLIF parameters
            if tau_per_neuron and reg_lambda != 0:
                for m in net.modules():
                    if isinstance(m, models.PLIFNode):
                        loss += reg_lambda * torch.var(m.w)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            if use_plif:
                ...
                for m in net.modules():
                    if isinstance(m, models.PLIFNode):
                        if tau_per_neuron:
                            log_data_list[-1].append(('w' + str(plif_idx), torch.mean(m.w.data).item(), net.train_times)) # Store mean of all parameters in each layer
                            log_data_list[-1].append(('w_std' + str(plif_idx), torch.std(m.w.data).item(), net.train_times)) # Store std. dev. of each layer
                        else:
                            log_data_list[-1].append(('w' + str(plif_idx), m.w.data.item(), net.train_times)) # Store tau of each layer
                        plif_idx += 1
            ...
            if (time.time() - ckpt_time) / 3600 > 4: # Starts saving every checkpoint after 4 hours of training - watch out in case the training is interrupted and resumed later
                # 检查点
                # 每4小时保存一次
                torch.save({
                    'net': net,
                    ...
                    'scheduler': scheduler.state_dict(),
                    'log_data_list': log_data_list
                }, os.path.join(pt_dir, f'checkpoint_{net.epoch}.pt'))
